...
from chapter13_prettytable.main import table

# table = PrettyTable
# table.field_names = [ "구분","부모 클래스", "자식클래스"]
...
'''
1. 상속이란?
   어떤 클래스가 가지고 있는 기능을 스대로 물려 받아서 사용할 수 있는 클래스를 생성할 수 있는데,
   클래스의 기능을 물려받을 꺠 상속받는다는 표현을 사용함.
   상속 관계에 있는 클래스를 표현할 떄 부모(슈퍼) 클래스 / 자식(서브) 클래스라는 용어를 사용함.
   
2. 상속 관계 구현
   두 클래스가 상속 관계레 놓이려면 IS-A 관계를 성립해야 함. IS-A광 계란 '~ㄴ는~이다'로 해석할 수 있는 
   관계를 의미
   ex) 학생은 사람이다.
...
# potter.study()   #자식 클래스의 매서드 호출

'''
3. 서브 클래스의 __init__()

서브 클래스는 슈처 클래스가 없으면 존재할 수 없습니다. 그러나 서브 클래스의 생성자를 수현 할 떄는
'반드시 슈퍼 클래스의 생성자를 먼저 호출' 하는 코드를 작성해야 합니다.

super -> 슈퍼 클래스를 의미. 즉, Student 의 생성자를 호출하려면 super().__init__(name) 에 의해서
수퍼 클래스인 Person 의 생성자가 먼저 호출되면서 슈퍼 클래스의 객체가 '생성'입니다.
이후 슈퍼 클래스에서 생성된 인스턴스 변수인 name이 서브 클래스로 전달되고 이후에 서브 클래스에서 school
인스턴스를 선언 및 초기화 하며 저장하면서 서브 클래승의 객체가 생성됩니다.

4. 서브 클래스의 인스턴스 자료형

슈퍼 클래스의 객체는 슈퍼 클래스의 인스턴스
하지만 서브 크래스와의 객체는 서브 클래스의 인스턴스 이면서 슈퍼 클래스이ㅢ 인스턴스
Sttudent 클래스의 객체는 Student의 인스턴스이면서 동시에 Person 의 인스턴스라는 의미입니다
...
지시사항
1. 다음과 같은 슈퍼 클래스 Car을 가지고 있는 Hybrid 클래스를 구형하세요.
...
class Car:
    ...
    def add_oil(self, oil):   #매서드
        ...
        self.oil +=oil
        if self.oil > Car.max_oil:  #주유 후 절대 주유량 초과라면
            self.oil = Car.max_oil    # 현재 주유량을 절대 주유량으로 대입
...
class Hybrid(Car):
    ...
    def hybrid(self):
        ...
        #print(f"현재 주유 상태 : {self.oil}")
        print(f"현재 충전 상태 : {self.amount}")   #super().을 통해서 부모 클래스의 매서드를 추출 가능
