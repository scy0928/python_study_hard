...
속성을 추가
 read()  메서드를 통해 전체를 읽응려면 메모리 공간이 많이 필요합니다. 읽어야 할 파일이 크다면
 일부만 읽어들이는 작업을 반복하는 반복문을 통해 파일 전테를 읽도록 구현하는 편이 좋습니다.
'''
# file = open("2025-02-26.txt", "rt")
# end_of_text = False
...
# file.close()
'''
이상의 코드는 30바이트씩 가지고 오게 되는데, 척번쨰  30바이트의 결과값 이루에 자동으로 개행이
일어납니다. (print() 함수의 기본기능 때문에 ) 그래서 이 부분을 파일과 콘솔에 정보가 동일하도록
end="" 를 적용한 사례입니다.

    2) readline() 매서드
       텍스트 파일을 한 줄씩 읽어서 처리한ㄴ 매서드
       만약 파일이 종료되어 더 읽어들일 데이터가 없다면 빈 ㅂ문자열(" " ) 을 읽어들입니다.
       반복문을 이용해서 여러 번 읽어들어야 할 떄  (한 줄씩) 파일 전테를 읽을 수 있습니다
'''
# file = open("2025-02-26.txt", "rt")
# str = file.readline()
# print(str)
...
#         print(line, end="")
'''
나라 별 수도를 순차적으로 반복시켜 nation 리스트에 사전에 미리 저장해두었습니다.
...
with open("nation.txt", "wt") as file:
    # list 딱수 번지 -> 국가 / 홀수 번지 -> 도시
    for i in range(0, ten(nation), 2):
        file.write(nation[1]+ " - " + nation[1+i] + "\n")