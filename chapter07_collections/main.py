...
'''
수학 여행을 어디로 갈 수 있을지 결정하기 위해 학생들이 희망하는 모든 수학 여행 장소를 조사하기로 했습니다. 학생들이 원하는 장소를 입력받아
동일한 입력을 무시하고 모든 입력을 저장하려고 합니다,
학생을 3명으로 가정하고 실행 예와 같이 동작하는 프로그램을 작성하였습니다.
...
#     field_trip_list = [student] #애플 사용할 경우 [student] 로 매번 초기화

# field_trip_set = set(field_trip_list)

# print(f"조사된 수학 여행지는 {field_trip_list}입니다.")
#print(f"조사된 수학 여행지는 '{field_trip_set}' 입니다.")
#print(f"조사된 수학 여행지는 '{list(field_trip_set)}' 입니다.")
...
# 몇 개의 숫자를 입력받을지 input작성 (-> 인덱스 넘버가 얼마짜리인 list를 만들 것인가를 묻는거죠)

# 인덱스 넘버의 숫자만큼 요소 를 입력하는 input 함수가 반복되어야 함

#그 요소에 해당하는 숫자가 홀수라면 두 번째 리스트에 저장되면 X 짝수라면 저장되어야 함. -> 얘는 조건문
# index_num = int(input(" 몇 개의 숫자를 입력할까요? >>>>>"))
# for i in range(index_num) :
#     num = int (input(f"{i+1}번쨰 숫자를 입력하세요 >>> "))
#     li_original.append(num)
#
# print(f"입력받은 숫자는 {li_original}입니다.")

#예를 들어 첫 번째 문자만 해결하고 짝수들만 뽑아내는 조건문을 작성하시겠다면
# li_original을 반복문 돌려서 요소들만 추출하고, 그 요소들이 홀/ 짝수 중 어디에 해당되는지
# 반복문+ 조건문으로 풀이할 필요가 있습니다.
# 원래대로라면 첫 번째 반복문 내에서 전부 해결할 수 있지만 향상된 for로 예시를 의해 반복문을
# 두번 돌리도록 수정하겠습니다

#향상된 for문
# for num in li_original:
#     if num % 2 == 8               # 꼭 else응 써야 하는 것은 아닙니다.
#         li_even.append(num)
#
# print(f"임의로 입력받은 숫자 중 짝수는 {li_even}입니다.")
...
#     num2 = int(input(f"{i+1}번쨰 숫자를 입력하세요 >>> "))
#     li_original2.append(num2)
#     if num2 % 2 ==0 :
#         li_even2.append(num2)  #그리고 그 숫자가 짝수라면 even2 에도 추가
#                                 #아니면 다음 반복으로
# print(f"입력받은 숫자는 {li_original2}입니다.")
# print(f"임의로 입력받은 숫자 중 짝수는 {li_even2}입니다.")
...
1 반쪠 시림의 전화번호를 입력하세요>>>> 010 - 1234 - 5678
2번째 사람의 이름을 입력하세요>>> 김이
2반쪠 시림의 전화번호를 입력하세요>>>> 010 - 2345 - 6789
3번째 사람의 이름을 입력하세요>>> 김삼
3반쪠 시림의 전화번호를 입력하세요>>>> 010 - 3456- 7890

입력받은 연락처는 '김일' :'010 - 1234 - 5678' . '김이' :'010 - 2345 - 6789' , '김삼' :'010 - 3456- 7890'
'''
# telephones = {}    # 비어있는 딕셔너리를 사용하는 방식
# for i in range(3):
#     dict_key = input( f"{i+1} 번째 사람의 이름을 입력하세요. >>>>> ")
#     dic_value = input(f"{i+1}번째 사람의 연락처를 입력하세요. >>>>> ")
#
# # 딕셔너리에 element 을 추가하는 방식
#     telephones[dict_key] = dict_values
#
# print(f"입력 받은 연락처는 {telepones} 입니다.")

'''
숫자를 입력한 횟수만큼 비어있는 list에 숫자를 추가하기
문제 : 비어있는 리스트 list01을 선언하고 그 안에 입력 받은 횟수만큼 숫자를 추가하십시오.
...
print(add_number2(last_num))    #call4 유형
...
# 이상의 코드는 가능하지만 --> add_numbers2(n)은 return으로 반환하기 때문에
#element에 대한 추가 연산 가능 -> 예) +1을 하건, 혹은 짝수만 걸러내면 etc...

# for number in add_numbers(last_num):
#     print(number+1)

# 이상의 코드는 불가능 합니다. -> add_numbers(n)의 경우 print 실행문으로 마무리 되기 때문에 추가 연산 X

'''
짝수와 홀수의 개수 세기
list 를 입력받아 짝수와 홀수의 개수를 세는 함수를 작성하시오

함수 정의 
함수 이름 : count_even_odd
매개 변수 : list numbers(요소는 모두 상수일 것)
...
#반복문 돌리고 거기 내부에  조건문 돌려서 짝수면 even-count에 1씩 더하고
