...
가방을 만들때마다 현재 만들어진 가방이 몇 개인지 계산할 수 있는 Bag 클래스를 정의합니다.
'''
#클래스 정의
# class Bag:
#
...
#
#     def __init__(self):  #생성자 / 인스턴스 매서드에 해당함 -> 아무 @가 없으므로
#         Bag.count += 1    #객체를 하나 생성할 떄마다 클래스 변수인 count가 1씩 증가함
#         print("가방이 생성되었습니다.")  #인스턴스 매서드가 클래스 변수를 참조할 떈,ㄴ
#                                        # 클래스명.클래스변수 명으로 접근함
#     #클래스 매서드 정의
#     @classmethod
#     def sell(cls):
...
#
#     @classmethod
#     def remain_bag(cls):     #클래스 변수를 main 단계에서 직접 참조하는 것이 보안 상 좋지 않기 떄문에
#         return  cls.count    #클래스 매서드 생성(glatter 형태로)하여 참조하는 방식
#
# print(f"현재 가방 재고 : {Bag.count}")    #클래스 변수를 직접 참조
# print(f"현재 가방 재고 : {Bag.remain_bag()}")   #클래스 변수를 getter를 통해 매서드를 경유하여 참조 -> 보안 상 더 좋음
#
# #객체 생성
# bag1 = Bag    #생성자 상기하셔야 하는대 아무런 속성이 없기 떄문에 앞에 argument를 채울 필요가 없습니다.
# print(f"현재 가방 재고 : {Bag.remain_bag()}") #클래스 변수를 클래스 매서드를 통해 조작할 수 있습니다.
# bag2 = Bag()
# bag3 = Bag()
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
# bag1.sell()   #인스턴스명.클래스매서드명()으로 호출 -> 가능했었죠 // 이경우는 사실 좀 애매하고
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
# Bag.sell()  #클래스명.클래스 매서드명()으로 호출 -> 더 권장되는 방식
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
'''
이상의 코드에서 보면 예시이기 떄문에 적합하다고 보기는 힘든데
인스턴스명.sell()로 작성한다면 특정한 가방 인스턴스가 팔렸다고 볼 수 있기 때문에 가독성이 더 높은 반면
Bag.sell()로 작성하게 된다면 어던 가방이 팔렸는지는 모르고 그녕 count변수만 하나 줄었음

저희는 코드할 때 변수 / 매서드를 인스턴스  수준으로 작성할지 혹은 클래스 수준으로 작성할지에 대한
고민이 실행되어야만 할겁니다.

응용 예제

1.다음 지시 사항을 읽고 이름과 전체 연구소를 저장 할 수 있는 Person 클래스를 작성하시오
...
3. 다음 코드를 통해서 전체 인구 수를 조회할 수 있도록 작성하십시오.
...
#     #소멸자 정의
#     def __del__(self):
#         Person.population -=1    #객체가 소명되었을 때 전체 인구수가 줄어야 한다는 점을 고려했을 때
...
대음 지시 사항을 읽고 가게의 매출을 구할 수 있는 Shop클래스를 작성하시오

지시사항
1. Shop 클래스는 가음과 같은 변수를 가지고 있다.
    total :가게 전체 매출액
...
Shop.sales("순대", 2)       #순대(을)를 2개 판매
Shop.sales("튀김", 5)       #튀김(을)를 5개 판매

3.모든 매출을 작성ㅇ한 뒤 다음과 같은 방식으로 전체 매출액을 확인합니다.
...
#생각 해보셔야 할게 menu_name을 argument로 받았을 떄, 걔가 리스트 내의 요소의 key 에 해당하는지
#확인해야 하고 그 key에 있는 value를 도출해서 quantity의 곱한 결과값을
#클래스 변수인 total에 더해줘야 전체 매출액이 변경됩니다
        ...
        for menu_dict in cls.menu_list:
            if menu_name in menu_dict:
                ...
                print(f"{menu_name}을 {quantity}에 판매")

       # 이상의 부분은 collection 의 요소들을 가져오는 연결문에 해당하는데
       # argument 로 menu_name을 받은 뒤에 그게 암시하는 key를 찾아내고 그것의 value * quantity해준 값을
       # total에 더해주면 되지 않을까요?
