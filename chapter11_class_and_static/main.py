...
#  들여다보기 전까지는 coutry라는 변수가 클래스 변수인지 인스턴스 변수인지 확인이
# 불가능 하다는 문제가 있습니다.

# 이상을 이유로 클래스 변수를 확인하고자 할 때는 객체명.클래스변수명으로 참조하기 보다는
# 클래스명.클래스변수명을 통해서 참조하는 것이 바람직합니다.
'''
클래스 메서드 : 클래스 변수를 사용하는 매서드
'''
class Korean2:
    country = "대한민국"    #ㅋ클래스 변수 선언 및 초기화

    #클래스 메서드 정의 법
    @classmethod         # 얘를 명시하면 앞의 코드가 클래스 메서드로 작성됨
...
man2 = Korean2()
man2.trip("일본")
#클래스 변수 때와 마찬가지오 격체명.클래스매서드명()으로 호출 가능
#-> 하지만 마찬갈지로 얘가 인스턴스 매서드인지 클래스 매서드인지 구분할 수 없기 때문에
#권장되는 코드 장성 요령이 아닙니다.