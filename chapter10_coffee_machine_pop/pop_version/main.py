...
#choice 가 "라떼" 라는 str 데이터라면, "라떼" 를 만드는데 필요한 재료를 조합하는 방법은
#무엇일까요?
...
def is_resources_enough(order_ingredients):
    """DocString : 함수/클래스/메서드가 어떤 장동을 하는지 '사람들에게' 설명해주는 기능
    주문 받은 음료를 resources에서 재료 처리를 하고 난 후 , 음료 만들기가 가능하며
    ...
    for item in order_ingredients:
       if order_ingredients[item] > resources[item]:
           ...
           return False
       return True # else 안 쓴 이유 : if문이 실행 안되었으면 어차피 True라서.

def process_coins():
    """동작들을 입력 받아 전체 금액을 반환하는 함수 call3()유형"""
    # 쿼터, 다임, 니켈, 패니 네 종류의 동전
    # 쿼터 = 0.25 달러
    # 다임 = 0.1달러
    # 니켈 = 0.05달러
    # 패니 = 0.01달러
    ...
    # dimes = int(input("디엠 동전을 몇 개 넣으시겠습니까? >>> ")) * 0.1
    # nickels = int(input("니켈 동전을 몇 개 넣으시겠습니까? >>> ")) * 0.05
    # pennies = int(input("패니 동전을 몇 개 넣으시겠습니까? >>> ")) * 0.01
    #
...
def is_transaction_successful(money_received, drink_cost):
    ...
    그리고 True이면 profit에 음료 가격만큼 추가해줘야 합니다. """

    charge = money_received - drink_cost
    if charge >= 0:
        #profit에 돈 추가해야 합니다 -> drink_cost만큼. profit -> 전액 변수
        global profit
        profit += drink_cost
...
while is_on:
    ...
    if choice == "off":
        ...
        # 1. 자원이 충분하지 않아서 T / F
        # 2. T라면 돈을 받아서 계산 -> 해당 금액의 가격보다 높은지 확인해서 T / F 반환
        # 3. T라면 음료를 만들어주는데, resources dict에 있는 수량을 감소 / 음료 가격만큼 + 시켜줘야겠네요.
        drink = MENU[choice]
        if is_resources_enough(drink["재료"]):
            # 이상의 조건성이 T라면 2. 단계로 넘어가야 함.
            money_received = process_coins()   #함수 호출한 결과값을 변수에 대입
            if is_transaction_successful(money_received,drink["가격"] ):
                # 재료 차감하고 음료 만들어서 사용자제게 제공
                # 재료 차감부분을 is_resources_enough()함수를 참고하여 작성하세요.
                # 여기에 작성하시고 추후에 함수화시킬 예정
                # 기본적으로 dict의 value를 뽑아내서 차감해야합니다.
                # resources["물"] -= drink["재료"]["믈"]
